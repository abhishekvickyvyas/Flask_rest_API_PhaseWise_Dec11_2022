...
class Get(Resource):
    def post(self):
        ...
        sentence_data = users.find({
            "Username": username
        })
        retJson = {
            "status":200,
            "sentence": str(sentence_data[0]["Sentence"]),
            "tokens": sentence_data[0]["Tokens"]
        }
...
class AddTokens(Resource):
    def post(self):
        ...
        users.update_one({
            ...
            "$set":{
                "Tokens":num_tokens+ int(tokens)
                }
        })

        retJson = {
            "status":200,
            "oldTokens": num_tokens,
            "updatedTokens": num_tokens+ int(tokens)
        }
        return retJson
...
api.add_resource(AddTokens, '/addToken')
